...
export const execCommand = async function({api, event, key, kernel, umaru, args, keyGenerator, Users, prefix, context, usage}) {
  ...
  return api.sendMessage({body: context, attachment: await kernel.readStream(["fingering2"], {key: key, senderID: await Users.getImage(male), targetID: await Users.getImage(female)})}, event.threadID, async() => {
