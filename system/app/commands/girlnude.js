...
export const execCommand = async function({api, event, kernel, umaru, translate}) {
        ...
        if(money <= 300) {
         return api.sendMessage((await translate("💵 You need 300$ to see the photo!", event, null, true)),event.threadID,event.messageID);
          ...
          await umaru.createJournal(event);
              return api.sendMessage({body: (await translate("Dads really like it😼\n» Costs: -300$ «", event, null, true)),attachment: await kernel.readStream(encodeURI(link[Math.floor(Math.random() * link.length)]))}, event.threadID, async() => {
